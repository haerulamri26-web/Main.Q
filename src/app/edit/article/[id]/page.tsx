...
export default function EditArticlePage() {
  ...
  const { data: article, isLoading: isArticleLoading } = useDoc<Article>(articleRef);
  ...
  const addLink = () => {
    ...
    if (url && url !== 'https://') {
      ...
      if (selection && selection.toString().length > 0) {
        ...
        const link = `<a href="${url}" target="_blank" style="color: hsl(var(--primary)); text-decoration: underline;">${url}</a>`;
  ...
  if (isUserLoading || isArticleLoading) return <div className="flex justify-center py-40"><Loader2 className="animate-spin h-10 w-10 text-primary" /></div>;
  if (!user || (article && user.uid !== article.userId)) return <div className="text-center py-20">Akses Ditolak</div>;

  const ToolbarButton = ({ command, value, icon: Icon, title, isActive, onClickOverride }: any) => (
    <Button 
      variant="ghost" size="icon" className={cn("h-8 w-8 rounded-md transition-colors", isActive && "bg-primary/10 text-primary hover:bg-primary/20")} 
      onMouseDown={(e) => { e.preventDefault(); onClickOverride ? onClickOverride() : executeCommand(command, value); }} 
      title={title}
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] pb-20">
      <style jsx global>{`
        .editor-area:empty:before { content: attr(data-placeholder); color: #9ca3af; cursor: text; }
        .editor-area a { color: hsl(var(--primary)); text-decoration: underline; }
        .editor-area h1 { font-size: 2rem; font-weight: bold; margin-bottom: 1rem; }
        .editor-area h2 { font-size: 1.5rem; font-weight: bold; margin-top: 1.5rem; margin-bottom: 0.75rem; }
      `}</style>

      <header className="bg-white border-b sticky top-0 z-50 px-4 py-3 shadow-sm">
        <div className="container mx-auto max-w-5xl flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button asChild variant="ghost" size="icon" className="rounded-full"><Link href="/profile"><ChevronLeft className="h-6 w-6" /></Link></Button>
            <div className="flex items-center gap-2">
              <div className="bg-primary/10 p-1.5 rounded-lg"><FileText className="h-5 w-5 text-primary" /></div>
              <span className="text-lg font-bold font-headline hidden sm:inline">Edit Artikel</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="outline" className="rounded-full px-6 hidden sm:flex" asChild><Link href="/profile">Batal</Link></Button>
      ...
      <main className="container mx-auto max-w-4xl mt-8 px-4 animate-in fade-in duration-500">
        ...
        <div className="bg-white rounded-2xl shadow-sm border mt-6 p-8 grid md:grid-cols-2 gap-8">
          <div className="space-y-3">
            <label className="text-sm font-bold text-slate-600 flex items-center gap-2"><Settings className="h-4 w-4" /> Kategori Artikel</label>
          ...
          <div className="space-y-3">
            <label className="text-sm font-bold text-slate-600 flex items-center gap-2"><Hash className="h-4 w-4" /> Label (Tag)</label>
            <Input 
        ...
        <div className="mt-6 flex justify-between items-center text-slate-400 text-xs px-2">
          <div className="flex gap-4">
            <span className="flex items-center gap-1.5"><Type className="h-3.5 w-3.5" /> {wordCount} kata</span>
            <span className="flex items-center gap-1.5"><FileText className="h-3.5 w-3.5" /> {charCount} karakter</span>
          </div>
