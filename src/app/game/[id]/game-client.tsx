...
export default function GameClient({ id }: { id: string }) {
  ...
  const commentsQuery = useMemoFirebase(() => {
    if (!firestore || !id) return null;
    return query(collection(firestore, 'publishedGames', id, 'comments'), orderBy('createdAt', 'desc'));
  }, [firestore, id]);
  ...
  useEffect(() => {
    if (game && typeof window !== 'undefined') {
        const subject = `Laporan Game: ${game.title} (ID: ${id})`;
  ...
  const handleCommentSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!firestore || !user || !id || !comment.trim()) return;

    setIsSubmitting(true);
    try {
      const commentsCollection = collection(firestore, 'publishedGames', id, 'comments');
    ...
    <>
      <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6 overflow-hidden">
        ...
        <Link href={`/?level=${game?.class.split(' ')[0]}`} className="hover:text-primary transition-colors shrink-0">
            {game?.class || <Skeleton className="h-4 w-12" />}
        </Link>
        <ChevronRight className="h-3 w-3 shrink-0" />
        <span className="text-foreground font-medium truncate">{game?.title || <Skeleton className="h-4 w-24" />}</span>
      </nav>
            ...
            <div className="md:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <div className="space-y-4 text-foreground/80 leading-relaxed">
                        <p>
                            Game ini selaras dengan semangat <strong>Kurikulum Merdeka</strong> yang mengedepankan pembelajaran berpusat pada siswa (<em>student-centered learning</em>). Beberapa manfaat utama meliputi pemahaman konsep yang lebih baik, interaktivitas tinggi, dan evaluasi mandiri yang menyenangkan.
                        </p>
                    </div>
                    )}
                </div>
