...
export default function RegisterPage() {
  ...
  const onEmailRegisterSuccess = (userCredential: UserCredential) => {
    toast({
      title: 'Pendaftaran Berhasil!',
      description: "Selamat! Silakan periksa email Anda untuk verifikasi.",
    });
    // The useEffect will handle the redirect.
  };

  const onGoogleRegisterSuccess = (userCredential: UserCredential) => {
    toast({
      title: 'Pendaftaran Berhasil!',
  ...
  const onEmailSubmit = (data: RegisterFormValues) => {
    if (!auth) return;
    setIsLoading(true);
    initiateEmailSignUp(
      auth,
  ...
  const handleGoogleSignIn = () => {
    ...
    initiateGoogleSignIn(auth, onGoogleRegisterSuccess, onRegisterError);
  };
    ...
    <div className="container mx-auto px-4 py-8 flex justify-center items-center min-h-[calc(100vh-250px)] animate-in fade-in-0 slide-in-from-top-4 duration-500">
      <Card className="w-full max-w-sm">
        <CardHeader>
          ...
          <CardDescription>
            Buat akun untuk mulai membuat dan berbagi game edukasi.
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit(onEmailSubmit)}>
          ...
          <CardFooter className="flex flex-col gap-4">
            <Button type="submit" className="w-full" disabled={isLoading || isGoogleLoading}>
              {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 'Buat Akun dengan Email'}
            </Button>
          </CardFooter>
        </form>
