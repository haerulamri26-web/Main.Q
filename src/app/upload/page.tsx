...
export default function UploadGamePage() {
  ...
  const onSubmit = (data: UploadFormValues) => {
    ...
    addDocumentNonBlocking(gamesCollection, gameData)
      .then(() => {
        toast({
          title: 'Berhasil!',
          description: `Game Anda "${data.title}" telah diunggah dan akan segera muncul.`,
        });
        router.push('/profile');
      })
      .catch((error) => {
      ...
      // We don't set isUploading to false in the 'finally' block because
      // we navigate away on success, so no need to re-enable the button.
  };
    ...
    <div className="container mx-auto px-4 py-8 animate-in fade-in-0 slide-in-from-top-4 duration-500">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Isi detail di bawah ini dan tempel kode HTML Anda untuk membagikan game Anda dengan dunia. Lihat halaman <a href="/tutorial" className="text-primary underline">tutorial</a> jika Anda butuh bantuan.
          </CardDescription>
        </CardHeader>
        <Form {...form}>
